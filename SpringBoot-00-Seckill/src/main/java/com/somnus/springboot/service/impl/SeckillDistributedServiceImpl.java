...
public class SeckillDistributedServiceImpl implements SeckillDistributedService {
	...
	@Override
	@Transactional
	public LogicResult<String> startSeckilRedisLock(long seckillId,long userId) {
		/**
		...
		if(isLock) {
			...
			return LogicResult.<String>builder().build().success("哎呦喂，人也太多了，请稍后！");
		}
	...
	@Override
	@Transactional
	public LogicResult<String> startSeckilZksLock(long seckillId, long userId) {
		boolean isLock = ZkLockUtil.acquire(3,TimeUnit.SECONDS);
		if(isLock) {
			try {
				//校验库存
				Seckill seckill = seckillMapper.selectByPrimaryKey(seckillId);
	...
	@Override
	@Transactional
	public LogicResult<String> startSeckilLock(long seckillId, long userId, long number) {
		boolean res=false;
		try {
